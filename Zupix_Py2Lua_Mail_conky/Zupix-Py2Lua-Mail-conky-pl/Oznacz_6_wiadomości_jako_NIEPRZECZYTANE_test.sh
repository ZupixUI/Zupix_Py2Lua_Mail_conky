...
for idx, acc in enumerate(accounts):
    try:
        with imaplib.IMAP4_SSL(acc["host"], acc["port"]) as imap:
            ...
            uids = data[0].split()
            if not uids:
                print(f"[NOTIFY]ℹ️ {acc['name']}: Brak wiadomości w INBOX.", flush=True)
                continue
            # Weź 6 najnowszych UID-ów (jak w Twoim przykładzie)
            latest_uids = uids[-6:]
            for uid in latest_uids:
                imap.store(uid, '-FLAGS', r'\\Seen')
            print(f"[NOTIFY]✅ {acc['name']}: Oznaczono jako nieprzeczytane: {len(latest_uids)} najnowszych wiadomości.", flush=True)
            imap.logout()
...
do
  ...
  --text="Oznaczanie 6 najnowszych wiadomości jako nieprzeczytane...\nProszę czekać." \
