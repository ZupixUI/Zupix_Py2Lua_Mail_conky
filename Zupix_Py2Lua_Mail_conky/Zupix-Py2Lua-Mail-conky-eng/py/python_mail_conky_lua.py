# -*- coding: utf-8 -*-
"""
Zupix_Py2Lua_Mail_conky – Persistent IMAP connections, multi-account
Copyright © 2025 Zupix, amator_80
GPL v3+

Maintains persistent IMAP sessions per account, periodically fetches new emails, does not relog in a loop.
"""
...
imaplib.IMAP4_SSL.timeout = 1  # Sets timeout for new imaplib connections
socket.setdefaulttimeout(1)    # Sets global timeout for sockets

max_mails = 20  # how many mails to fetch per account (you can change)

def load_accounts_json():
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    config_path = os.path.join(base_dir, "config", "accounts.json")
    if not os.path.exists(config_path):
        print(f"Accounts file missing: {config_path}")
        sys.exit(1)
...
CACHE_WRITE_INTERVAL = 1  # how many seconds between cache file writes (float, e.g. 0.5, 2)
UPDATE_INTERVAL = 1       # how many seconds between fetches per account

# --- DECODING AND UTILITIES ---
...
def line_priority(line):
    l = line.lower().strip()
    greetings = [
        "good morning", "hello", "hi", "dear", "hej", "witam", "dzień dobry", "cześć", "witaj"
    ]
    for greet in greetings:
        if l.startswith(greet):
            return 100
...
# --- MAIN MAIL FETCHING LOGIC – ONE THREAD PER ACCOUNT ---
...
class AccountWorker(threading.Thread):
    ...
    def connect(self):
        try:
            ...
            self.connected = True
            self.last_error = None
            print(f"[{self.account['name']}] Connected to IMAP.")
        except Exception as e:
            self.connected = False
            self.last_error = f"[Account error {self.account['name']}] {e}"
            print(f"[{self.account['name']}] Connection error: {e}")
    ...
    def run(self):
        ...
        while True:
            if self.connected:
                try:
                    print(f"[{self.account['name']}] NOOP trying…")
                    self.imap.noop()
                    ...
                    traceback.print_exc()
                    self.connected = False
                    self.last_error = f"[Account error {self.account['name']}] Connection dropped (NOOP): {e}"
                    try:
                        self.imap.logout()
                    except:
                        pass
            ...
            if not self.connected:
                self.connect()
                if not self.connected:
                    self.last_error = f"[Account error {self.account['name']}] No connection to the internet or IMAP server"
            ...
            try:
                ...
                self.unread = unread
                self.mails = mails
                self.last_error = None  # <-- ALWAYS CLEAR ERROR ON SUCCESSFUL FETCH
                print(f"[{self.account['name']}] Mail fetch OK ({self.unread} unread)")
            except Exception as e:
                print(f"[{self.account['name']}] MAIL FETCH ERROR: {repr(e)}")
                traceback.print_exc()
                self.last_error = f"[Account error {self.account['name']}] {e}"
                try:
                    self.imap.logout()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cache-interval", type=float, default=CACHE_WRITE_INTERVAL,
        help="How many seconds between cache file writes (default: 3)")
    ...
    while True:
        ...
        if now - last_write >= CACHE_WRITE_INTERVAL:
            ...
            # log cache file creation time (mtime)
            try:
                stat = os.stat(final_cache_file)
                mtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(stat.st_mtime))
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Created {final_cache_file} | file mtime: {mtime}", flush=True)
            except Exception as e:
                print(f"Error getting mtime: {e}")
            last_write = now
        time.sleep(1)  # or less, as needed (e.g. 0.1)
